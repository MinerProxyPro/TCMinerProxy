...
配套的本地安全客户端 [TMS](https://github.com/MinerProxyPro/TMS) 可用于加密与压缩传输数据，在降低带宽压力的同时增强链路安全性。开始使用前，请先阅读 [服务协议](https://rustminersystem.gitbook.io/rustminersystem/guan-yu/fu-wu-xie-yi)。

## 核心能力

| 能力 | 说明 |
| --- | --- |
| 传统矿池转发代理 | 一站式对接各类主流传统矿池，集中管控全网矿机连接通道、端口分配与流量转发规则，简化批量矿机运维。 |
| 私有矿池节点搭建 | 支持自主部署专属矿池节点，适配节点服务商、大型实体矿场、自有算力业务等私有化运营场景。 |
| 灵活自定义抽水费率 | 可按需自主设定收益抽水比例，灵活拓宽矿场、节点服务商的运营盈利空间 |
| 全链路加密安全传输 | 搭配配套 TMS 本地客户端，实现数据加密、流量压缩、通信链路三重防护，杜绝算力劫持、数据窃听风险。 |
| 全平台兼容部署方案 | 适配 Linux、Windows、ARM、ARMV7 多架构设备，提供对应一键部署脚本与完整安装包，跨设备快速落地。 |
| 可视 Web 管理后台 | 浏览器直接登录可视化管控面板，实时查看程序运行状态、端口占用、在线矿工、算力流水等全维度数据。 |
